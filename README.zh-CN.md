...
它就活在你终端的状态栏里 —— Boss、token 余量、连胜、下一个任务,一眼尽收。那个 **`[HUD]`** 是个可点链接,直通实时竞技场:

![终端里的 Slime HUD —— Cmd/Ctrl+点击 [HUD] 打开竞技场](docs/media/open-hud.png)

## 全景体验

在终端里写码,在浏览器里看打架 —— 同一个真实 session,对工作零影响。状态栏始终盯着 Boss 和你的余量,竞技场把每一击完整演给你看。

![Slime —— 终端与竞技场并排](docs/media/full-experience.png)
...
| 🔼 **Tokens** | ↑ 上行(输入/上下文)与 ↓ 下行(输出)token 数 |
| 🔥 **Streak** | 连续出击天数(括号里是最佳记录) |

## Token 流动

竞技场顶部两条进度条,把你的 token 消耗变成一场实时燃烧:**↑ 上行**(输入/上下文)不断填充、升温 —— 金 → 橙 → 🔥 红,燃烧边缘有火苗,快满时红色脉冲;**↓ 下行**(模型输出)在下方以冷青色流动。每条都带实时数值,每回合烧掉的 token 会从边缘溅出火星。

![Slime token 流动 —— 上行与下行进度条在燃烧](docs/media/token-bars.gif)

## 周报 Wrapped

跑 `/slime:wrapped` 看你过去 7 天的战报 —— 还会在状态目录旁生成一张**可分享的战绩卡**（`wrapped.svg`），随手就能晒：

![Slime 周报 —— 可分享的战绩卡](docs/media/wrapped-card.png)
...
**HUD 会自己启用。** Slime 的「会话开始」钩子会自动把状态栏写进你的设置,所以下次开 session 游戏就直接在那了 —— 不用手动设置。(已经有状态栏,或想手动配?跑 `/slime:setup`。想关掉自动 HUD,在 `~/.claude/slime/config.json` 里设 `"autoHud": false`。)

打开自动更新,这样每次改进都能到你这:`/plugin` → Marketplaces → slime → Enable auto-update(第三方 marketplace 默认是关的)。

就这样 —— 照常工作即可,游戏会自己玩起来。

## 第一次运行 —— 打开竞技场

下次开 session,HUD 就出现在终端底部(如上图)。想在浏览器里打开完整竞技场,**Cmd+点击**(macOS)或 **Ctrl+点击**(Windows / Linux)那个 **`[HUD]`** 链接 —— 或者跑 **`/slime:arena`**。把那个标签页摆在终端旁边,一边干活一边看打架。

## 排错

- **`/plugin` 用不了?** Claude Code 太旧了 —— 更新(`brew upgrade claude-code` 或 `npm i -g @anthropic-ai/claude-code@latest`)再重启。
- **HUD 没出现?** 跑 `/reload-plugins`(或重启 Claude Code)。如果你本来就有状态栏,Slime 不会覆盖它 —— 跑 `/slime:setup` 切换。
- **「找不到插件」/ marketplace 过期?** `/plugin marketplace update slime`,再重装。
- **`[HUD]` 链接点不开 / 竞技场打不开?** 本地竞技场服务在首次 session 启动;跑 **`/slime:arena`** 重新拉起。
- **想关掉自动 HUD:** 在 `~/.claude/slime/config.json` 里设 `"autoHud": false`。
...
同时开了好几个终端?竞技场顶栏会出现 📡 选台器 —— 钉住任意一场战斗(钉在 URL 里,每个浏览器标签可以看不同的 session),或让它自动跟随最新的。
