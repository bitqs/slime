...
/** Human-readable reward lines (levelup, then one per new badge) for a
 *  recordDefeat result. Single source so the /defeat console output and the
 *  arena event text can never drift.
 *  @param {{leveledUp:boolean,level:number,titleKey:string,newBadges:string[]}} r @param {string} lang @returns {string[]} */
...
module.exports = { emitRewards, rewardLines };
